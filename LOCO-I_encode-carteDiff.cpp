  ...
  for (int i = 0; i < nH; i++) {
    for (int j = 0; j < nW; j++) {
        if (i > 0 && j > 0) {
            ...
            if (b >= std::max(a, c)) {
                ImgTemp[i*nW+j] = std::min(a, c);
            }
            else if (b <= std::min(a, c)) {
                ImgTemp[i*nW+j] = std::max(a, c);
            }
            else {
                ImgTemp[i*nW+j] = a + c - b;
            }
        }
        else if (i == 0 && j != 0) {
            ImgTemp[i*nW+j] = ImgIn[i*nW+(j-1)];
        }
        else if (j == 0 && i != 0) {
            ImgTemp[i*nW+j] = ImgIn[(i-1)*nW+j];
        }
        else if (i == 0 && j == 0) {
   ...
   for (int i = 0; i < nH; i++) {
    for (int j = 0; j < nW; j++) {
        ImgOut[i*nW+j] = (ImgTemp[i*nW+j] - ImgIn[i*nW+j]) + 128;
    }
   }
